...
foreach city in $c40_cities {
	...
	*open the city file
	use "tmp_$cityBounds/`city'_target2.dta", clear

	*generate a bunch of vars to store summary info
	foreach var of varlist $ns_metrics pop {
		summ `var', d
		gen `var'_N= r(N) 
	...
	*get summary stats for t2
	...
	savesome city ndvi_* ga_* gba_* mndvi_* pop_* t2 if _n==1 using "tmp_$cityBounds/`city'_summary_t2.dta", replace
}
