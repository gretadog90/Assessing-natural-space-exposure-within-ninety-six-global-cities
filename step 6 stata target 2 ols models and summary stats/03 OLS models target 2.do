...
foreach city in $c40_cities {
	append using "tmp/`city'_target2.dta"
}

save "tmp/target2_full.dta", replace
...
foreach city in $c40_cities {
	...
	*save summary data set that has model output info
	savesome city b_* se_* adjr2* rmse* mndvip* if city=="`city'" & row==1 using "tmp/ols_`city'_summary_t2.dta", replace
}
...
foreach city in $c40_cities {
	append using "tmp/ols_`city'_summary_t2.dta"
}

assert _N==$cityCount


save "output/ols_summary_t2.dta", replace
outsheet using "output/ols_summary_t2.csv", comma replace
