...
foreach city in $c40_cities {
	append using "tmp/`city'_target1.dta"
}

save "tmp/target1_full.dta", replace
...
foreach city in $c40_cities {
	...
	*save summary data set that has model output info
	savesome city b_* se_* adjr2 rmse ndvip* if city=="`city'" & row==1 using "tmp/ols_`city'_summary_t1.dta", replace
}
...
foreach city in $c40_cities {
	append using "tmp/ols_`city'_summary_t1.dta"
}

assert _N==$cityCount


save "output/ols_summary_t1.dta", replace
outsheet using "output/ols_summary_t1.csv", comma replace
...
outsheet using "output/thresholds_t1.csv", comma replace

save "output/thresholds_t1.dta", replace
